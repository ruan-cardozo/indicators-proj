...
export class RabbitMQWorker {
	...
	async handleMetricMessage(message: MetricMessage) {
		const { projectId, metricData } = message;
		...
		try {
			...
			if (metricData.indentation) {
				const analysis = queryRunner.manager.create(
					IndentationAnalysis,
					{
				...
				for (const file of metricData.indentation.files || []) {
					const indentationFile = queryRunner.manager.create(
						IndentationFile,
						{
					...
					const savedFile =
						await queryRunner.manager.save(indentationFile);

					for (const dist of file.stats.indentDistribution || []) {
						const indentDist = queryRunner.manager.create(
							IndentDistribution,
							{
								level: dist.level,
								count: dist.count,
			...
			return { success: true, metricId: savedMetric.id };
		} catch (error) {
